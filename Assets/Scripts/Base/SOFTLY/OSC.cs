...
//{
  ...
  public class OSC : MonoBehaviour
  {
	  ...
	  private void Update() {
		...
		if ( messagesReceived.Count > 0 ) //Debug.Log("received " + messagesReceived.Count + " messages");
			lock(ReadThreadLock) {
				foreach (OscMessage om in messagesReceived)
				{
					...
					if ( al != null)
						foreach (OscMessageHandler h in al) h(om);
				}
      ...
      switch ( (char)packet[ start ] )
      {
        ...
        case '#':
          ...
          if ( bundleString == "#bundle" )
          {
            ...
            while ( index < length )
            {
              var messageSize = ( packet[index++] << 24 ) + ( packet[index++] << 16 ) + ( packet[index++] << 8 ) + packet[index++];
    ...
    private static int ExtractMessage(ArrayList messages, byte[] packet, int start, int length)
    {
      ...
      foreach (var c in typeTag)
	      switch (c)
	      {
		      ...
		      case 'i':
		      {
			      var i = ( packet[index++] << 24 ) + ( packet[index++] << 16 ) + ( packet[index++] << 8 ) + packet[index++];
			      oscM.values.Add(i);
			      break;
		      }
		      case 'f':
