...
namespace thriftImpl
{
    public class CommandSender : Sender
    {
            ...
            GameObject gameCharacterLeftFoot = null, GameObject gameCharacterRightFoot = null)
        {
            if (isConnected())
            {
                var position = gameCharacter.transform.position;
                var orientation = gameCharacter.transform.rotation;
                var scale = gameCharacter.transform.localScale;
        ...
        private void SendCharacterMemberMessage(string gameCharacterName, string characterMember,
            GameObject gameCharacterMember)
        {
            if (isConnected())
            {
                var position = gameCharacterMember.transform.position;
                var orientation = gameCharacterMember.transform.rotation;
                var scale = gameCharacterMember.transform.localScale;
                var shift = orientation * new Vector3(0.5f * scale.x, -0.5f * scale.y, -0.5f * scale.z);
        ...
        /// <param name="gazeInfluence">gaze influence with which to gaze at the object</param>
        private void SendObjectMessage(GameObject gameObject, bool gaze = false,
            GretaObjectTracker.Influence gazeInfluence = GretaObjectTracker.Influence.EYES)
        {
            if (isConnected())
            {
