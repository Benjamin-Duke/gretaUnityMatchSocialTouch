...
namespace TactilePerception
{
    ...
    public class HandPerceptionPropertiesManager : MonoBehaviour
    {
        //[SerializeField] [Tooltip("Reference to the TouchPerceptionManager associated with this hand.")]
        private TouchPerceptionManager _touchManager;
        ...
        void Start()
        {
            _touchManager = GetComponentInParent<TouchPerceptionManager>();
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            var colliderOther = collision.GetContact(0).otherCollider;
            if (!colliderOther.CompareTag("CamilleCollision") || colliderOther.transform != collision.collider.transform) return;
            var localPosition = ComputeLocalPosition(collision.collider.transform, out var anatomyParameters);
            if (_touchManager.enabled)
                _touchManager.PartCollisionEnter(collision, largestVelocity, transformVelocity, gameObject, localPosition, anatomyParameters);
        }
        
        private void OnTriggerEnter(Collider other)
        {
            if (!other.CompareTag("CamilleEtheral")) return;
            if (_touchManager.enabled)
                _touchManager.EtherealTriggerEnter(other, gameObject);
        }
        ...
        private void OnCollisionStay(Collision collision)
        {
            ...
            var localPosition = ComputeLocalPosition(collision.collider.transform, out var anatomyParameters);
            if (_touchManager.enabled)
                _touchManager.PartCollisionStay(collision, transformVelocity, gameObject, localPosition, anatomyParameters);
        }

        private void OnCollisionExit(Collision collision)
        {
            if (!collision.collider.CompareTag("CamilleCollision")) return;
            if (_touchManager.enabled)
                _touchManager.PartCollisionExit(collision, transformVelocity, gameObject);
        }
        ...
        private void OnTriggerExit(Collider other)
        {
            if (!other.CompareTag("CamilleEtheral")) return;
            if (_touchManager.enabled)
                _touchManager.EtherealTriggerExit(other, gameObject);
        }
